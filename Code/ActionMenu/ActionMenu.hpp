...
public:
	...
	void addAction(std::string const& p_action, std::string const& p_shortcut = "")
	{
		...
		actionMenuItem->mouseUpListeners += [this](AvoGUI::MouseEvent const& p_event) {
			if (p_event.mouseButton == AvoGUI::MouseButton::Left && p_event.target->getSize().getIsContaining(p_event))
			{
				actionMenuItemChoiceListeners(p_event.target);
	...
	{
		if (p_anchorX + m_targetBounds.getWidth() > getParent()->getWidth() - MIN_PARENT_MARGIN)
		{
			m_targetBounds.setRight(p_anchorX);
			if (p_anchorY + m_targetBounds.getHeight() > getParent()->getHeight() - MIN_PARENT_MARGIN)
			{
